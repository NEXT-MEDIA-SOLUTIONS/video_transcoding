...
LIBMEDIAINFO_PATH=Env.get("LIBMEDIAINFO_PATH")
...
class VideoInfo:
    ...
    def check_frame_accuracy(self, expected_duration) -> int:
        ...
        if "frame_count" in self.video_stream and "frame_rate" in self.video_stream:
            ...
            if flag and stdout:
                print(stdout)
                r_frame_rate, frame_count = stdout.strip().strip(",").split(',')
                print(r_frame_rate, frame_count)
    ...
    def get_media_info(self):
        media_info = MediaInfo.parse(self.file_path, library_file=LIBMEDIAINFO_PATH) 
